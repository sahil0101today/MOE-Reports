...
def process_url_and_recipients(url, recipients, email, password):
    ...
    try:
        ...
        if len(sms_df) > 0:
            ...
            sms_df_column_order = ["Channel", "Sent", "Total Delivered", "Delivery Rate", "Clicks", "Click Rate"]
            sms_df = sms_df[sms_df_column_order]
            sms_df = sms_df.rename(columns={"Sent": "Total Sent", "Clicks": "Total clicks"})
        else:
            sms_df = pd.DataFrame(columns=sms_df_column_order)  # Create an empty DataFrame if no data
        ...
        if len(whatsapp_df) > 0:
            ...
            whatsapp_df_column_order = ["Channel", "Total Sent", "Total Delivered", "Delivery Rate", "Total Read", "Open Rate", "Total clicks", "Click Rate", "Unique clicks", "Unique Click Rate"]
            whatsapp_df = whatsapp_df[whatsapp_df_column_order]
            whatsapp_df = whatsapp_df.rename(columns={"Total Read": "Total Open"})
        else:
            whatsapp_df = pd.DataFrame(columns=whatsapp_df_column_order)  # Create an empty DataFrame if no data
        ...
        if len(pn_df) > 0:
            ...
            pn_df_column_order = ["Channel", "All Platform Sent", "All Platform Impressions", "Delivery Rate", "All Platform Clicks", "Click Rate"]
            ...
            pn_df = pd.DataFrame(columns=pn_df_column_order)  # Create an empty DataFrame if no data

        #########################################################################################################################################

        # Concatenate all dataframes
        combined_df = [email_df, sms_df, whatsapp_df, pn_df]
